...
FILES_TO_LINK := .vimrc .zshrc .gitconfig .gitignore_global .tmux.conf .zsh/.git-prompt.sh .codex/config.toml .codex/hooks.json .config/herdr/config.toml
...
.PHONY: install link unlink commit
...
link_home_files:
	...
	@$(foreach file,$(FILES_TO_LINK),mkdir -p "$(dir $(HOME_DIR)/$(file))"; ln -sfn "$(CURRENT_DIR)/$(file)" "$(HOME_DIR)/$(file)";)
...
link_vscode_files:
	...
	@$(foreach file,$(VSCODE_FILES),ln -sfn "$(CURRENT_DIR)/vscode/$(file)" "$(VSCODE_SETTING_DIR)/$(file)";)
